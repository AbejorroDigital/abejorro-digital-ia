...
export function appendMessage(role, content) {
    ...
    wrapper.className = `flex gap-3 md:gap-6 ${role === 'user' ? 'justify-end' : 'animate-fade-in'}`;
    ...
    wrapper.innerHTML = `
        ...
        <div class="relative group max-w-[88%] md:max-w-[80%]">
            ...
            ${role === 'ai' ? `
                <button onclick="window.copyToClipboard(this)" class="absolute -bottom-7 right-2 text-[10px] text-gray-500 hover:text-yellow-500 flex items-center gap-1.5 transition-all font-bold uppercase tracking-wider">
...
export function showLoadingMessage() {
    const chatBox = document.getElementById('chatBox');
    const wrapper = document.createElement('div');
    wrapper.className = 'flex gap-3 md:gap-6 animate-fade-in';
